...
def normalize_resource_text_field(value: Any, field_name: str) -> str:
...
def normalize_resource_drive_type(drive_type: str) -> str:
    normalized = normalize_resource_text_field(drive_type, "网盘类型").lower()
    mapping = {
        '夸克': 'quark',
        '夸克网盘': 'quark',
        'quark': 'quark',
    ...
    return normalize_resource_text_field(drive_type, "网盘类型")
...
def extract_baidu_pwd_from_url(resource_url: str) -> str:
    try:
...
def detect_resource_drive_and_url(content: str, preferred_drive_type: Optional[str] = None) -> Tuple[str, str]:
    ...
    drive_types = (
        [preferred_drive_type]
        if preferred_drive_type and preferred_drive_type in RESOURCE_URL_PATTERNS
        else list(RESOURCE_URL_PATTERNS.keys())
    )
...
def validate_resource_link_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    known_drive_type = drive_type if drive_type in RESOURCE_URL_PATTERNS else None

    try:
        detected_drive_type, resource_url = detect_resource_drive_and_url(raw_resource_url, known_drive_type)
    except ValueError:
    ...
    if detected_drive_type and known_drive_type and detected_drive_type != known_drive_type:
        raise ValueError("网盘类型与资源链接不匹配")
    if detected_drive_type and not known_drive_type:
    ...
    for item in items:
        ...
        if existing:
            duplicates.append({
                ...
                "new_resource_type": item.get('resource_type', ''),
...
def get_resource_metadata_export_updated_at(resource: Dict[str, Any]) -> Optional[datetime]:
    raw_value = str(resource.get('updated_at') or resource.get('created_at') or '').strip()
    if not raw_value:
        return None
...
def format_resource_export_meta(resource_group: Dict[str, Any]) -> str:
    ...
    if update_mode == 'weekly':
        ...
        if weekday_text:
            return f"每周{weekday_text}更1集"
        return "周更"
    elif update_mode == 'daily':
        return f"日更{daily_episode_count}集" if daily_episode_count > 0 else '日更'
    elif update_mode == 'interval':
        return f"每{interval_days}天更1集" if interval_days > 0 else '固定更新'
...
def build_resource_links_export_document(resource_links: List[Dict[str, Any]], empty_message: str = "当前暂无卡密资源") -> str:
    ...
    type_order = {name: index for index, name in enumerate(RESOURCE_TYPE_OPTIONS)}
    ...
    for resource_group in grouped_by_name.values():
        ...
        resource_group['links'].sort(
            key=lambda item: (
                drive_order.get(item['drive_type'], 9),
                get_resource_drive_label(item['drive_type']).lower(),
                item['resource_url'],
            )
        )
    ...
    if today_hot_resources:
        lines.append(f"### 热门更新({today.month}.{today.day})")
...
def build_resource_copywriting_export_text(
    ...
    for link in resource_links:
        ...
        if latest_episode > int(resource_group.get('latest_episode') or 0):
            resource_group['latest_episode'] = latest_episode

        updated_at_dt = get_resource_link_export_updated_at(link)
        if updated_at_dt and (
            resource_group['latest_updated_at_dt'] is None
            or updated_at_dt > resource_group['latest_updated_at_dt']
        ):
            resource_group['latest_updated_at_dt'] = updated_at_dt
    ...
    for index, resource_group in enumerate(resource_groups):
        ...
        for link in sorted_links:
            ...
            if not resource_url:
                continue
            drive_label = get_resource_drive_label(link['drive_type']) or '网盘'
...
def is_frontend_document_request(request) -> bool:
    """识别浏览器直接刷新前端页面的文档请求，避免命中同名后端接口。"""
    if request.method != 'GET':
        return False

    if request.url.path not in FRONTEND_API_CONFLICT_PATHS:
        return False
...
async def log_requests(request, call_next):
    ...
    if is_frontend_document_request(request):
    ...
    try:
        ...
        if normalized_export_range == 'since_last':
            last_export_setting = db_manager.get_user_setting(
                current_user['user_id'],
                'resources_copywriting_last_exported_at'
            )
        ...
        log_with_user('error', f"导出资源文案失败: {str(e)}", current_user)
        raise HTTPException(status_code=500, detail=str(e))
...
def get_resource_links(
    ...
    try:
        ...
        normalized_resource_type = normalize_resource_type(resource_type) if resource_type else None
        resource_links = db_manager.list_resource_links(
            current_user['user_id'],
            keyword,
...
def export_resource_links_document(
    ...
    try:
        ...
        filename = f"{filename_prefix}_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
        encoded_filename = quote(filename.encode('utf-8'))
        export_finished_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def create_delivery_rule(rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        rule_id = db_manager.create_delivery_rule(
            ...
            user_id=user_id,
            delivery_mode=rule_data.get('delivery_mode', 'card'),
...
def update_delivery_rule(rule_id: int, rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        success = db_manager.update_delivery_rule(
            ...
            description=rule_data.get('description'),
            user_id=user_id,
            delivery_mode=rule_data.get('delivery_mode'),
            document_type_id=rule_data.get('document_type_id')
        )
        if success:
